...
export function renderModernClean({ profile, content, watermark, brandName }) {
  const header = `
    <div class="hdr">
      ${avatarHtml(profile.photoDataUrl)}
      <div>
        <div class="name">${safe(profile.fullName)}</div>
        <div class="sub muted">${safe(content.headline || profile.targetRole || '')}</div>
      </div>
      <div class="contact">
  ...
  const css = `
    .hdr { display:flex; align-items:flex-start; justify-content:space-between; gap:16px; padding-bottom:10px; border-bottom:1px solid #e5e7eb; }
    .name { font-size: var(--title-font); font-weight: 700; letter-spacing: -0.02em; }
    .sub { margin-top: 4px; font-size: 13px; }
    .contact { text-align:right; font-size: 12px; color:#333; margin-top:2px; }
  `;
