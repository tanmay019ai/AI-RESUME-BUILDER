import { avatarHtml, li, safe, section, watermarkHtml, wrapDocument } from './helpers.js';

export function renderTechDeveloper({ profile, content, watermark, brandName }) {
  const header = `
    <div class="hdr">
      ${avatarHtml(profile.photoDataUrl)}
      <div>
        <div class="name">${safe(profile.fullName)}</div>
        <div class="mono muted">${safe(content.headline || profile.targetRole || '')}</div>
      </div>
      <div class="contact">
        <div class="mono">${safe(profile.email)}</div>
        <div class="mono">${safe(profile.phone)}</div>
      </div>
    </div>
  `;

  const exp = (content.experience || []).map((e) => `
    <div class="item avoidBreak">
      <div class="row">
        <div><strong>${safe(e.title)}</strong> @ ${safe(e.company)}</div>
        <div class="right muted">${safe(e.startDate)} - ${safe(e.endDate)}</div>
      </div>
      <div class="muted mono">${safe(e.location)}</div>
      <ul>${li(e.bullets || [])}</ul>
    </div>
  `).join('');

  const pro = (content.projects || []).map((p) => `
    <div class="item avoidBreak">
      <div class="row">
        <div><strong>${safe(p.name)}</strong>${p.tech?.length ? ` <span class="muted mono">[${safe(p.tech.join(', '))}]</span>` : ''}</div>
      </div>
      <ul>${li(p.bullets || [])}</ul>
    </div>
  `).join('');

  const edu = (content.education || []).map((ed) => `
    <div class="item avoidBreak">
      <div class="row">
        <div><strong>${safe(ed.school)}</strong> — ${safe(ed.degree)}</div>
        <div class="right muted">${safe(ed.startDate)} - ${safe(ed.endDate)}</div>
      </div>
      <ul>${li(ed.details || [])}</ul>
    </div>
  `).join('');

  const body = `
    ${watermarkHtml({ watermark, brandName })}
    <div class="page">
      ${header}
      <div class="grid">
        <div>
          ${section('Summary', `<p>${safe(content.summary || '')}</p>`)}
          ${section('Experience', exp || '<p class="muted">—</p>')}
          ${section('Projects', pro || '<p class="muted">—</p>')}
        </div>
        <div>
          ${section('Skills', `<div class="box mono">${safe((content.skills || []).join(' • '))}</div>`)}
          ${section('Education', edu || '<p class="muted">—</p>')}
        </div>
      </div>
    </div>
  `;

  const css = `
    .mono { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace; }
    .hdr { display:flex; justify-content:space-between; gap:16px; border:1px solid #111; padding:12px 14px; border-radius: 10px; }
    .name { font-size: 22px; font-weight: 800; }
    .contact { text-align:right; font-size: 12px; }
    .grid { margin-top: 12px; display:grid; grid-template-columns: 2fr 1fr; gap: 14px; }
    .box { border: 1px solid #111; border-radius: 10px; padding: 10px; }
    @media (max-width: 720px) { .grid { grid-template-columns: 1fr; } }
  `;

  return wrapDocument({ css, body });
}
